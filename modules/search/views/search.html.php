...
<div id="g-search-results">
  ...
  <? if ($album->id == 1) { ?>
  <div>
    <?= t("Searched whole gallery") ?>
  </div>
  <? } else { ?>
  <div>
    <?= t("Searched within album %album", array("album" => html::purify($album->title))) ?>
    <a href="<?= url::site(url::merge(array("album" => 1))) ?>"><?= t("Search whole gallery") ?></a>
  </div>
  <? } ?>
